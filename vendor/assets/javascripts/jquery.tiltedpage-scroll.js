...
!function($){
  ...
  $.fn.tiltedpage_scroll = function(options){
    ...
    el.find('.tps-section').each(function(){
      ...
      el2.wrapInner("<div class='tps-wrapper' data-origin-offset-top='" + top + "'></div>");
    });
    ...
    $(window).on('DOMContentLoaded load resize scroll', function() {
      ...
      el.find('.tps-section.tps-inview > .tps-wrapper').each(function(index){
        var el2 = $(this),
            ...
            deg = ((el.offset().top - el2.parent().height()) - st + (el2.parent().offset().top - el.offset().top)) / $(window).height() * (settings.angle * 3),
            scale = ((st + $(window).height() - (el.offset().top - el2.parent().height()) - (el2.parent().offset().top - el.offset().top)) / ($(window).height() )) ;
            if(scale > 1) scale = 1;
            if(deg < 0) deg = 0;

        // if(st > el2.parent().offset().top) {
        if(el2.parent().offset().top >= el2.data('origin-offset-top')) {
          if (settings.outAnimation == false) {
            ...
            console.log(el2.parent().offset().top, el2.data('origin-offset-top'));
            opacity = ((el.offset().top + (el2.parent().offset().top - el.offset().top) + ($(window).height() * 1.2) - st)) / ($(window).height());
            opacity = Math.pow(opacity,25);
            // deg = (el2.parent().offset().top - st) / $(window).height() * (settings.angle * 3);
            // deg = (el.offset().top + (el2.parent().offset().top - el.offset().top) - st) / $(window).height() * (settings.angle * 3);
            // scale = ((st + $(window).height() - el2.parent().offset().top ) / ($(window).height() )) ;
            scale = ((st + $(window).height() - el.offset().top - (el2.parent().offset().top - el.offset().top) ) / ($(window).height() )) ;
          }
        } else {
        	// // if(index != 0) {
          // //   // var opacity = ((st + $(window).height() - el2.parent().offset().top + (el2.height()/2))/ $(window).height());
          // //   var opacity = ((st + $(window).height() - el.offset().top - (el2.parent().offset().top - el.offset().top)+ (el2.height()/2))/ $(window).height());
